...
def data_choose(args, block):
    ...
    if args.data=='intention':
        workers = args.worker
        data_set_train = intentionData(**args.data_param.train_data_param)
        data_set_val = intentionData(**args.data_param.val_data_param)
        cf = cfv = None
        shutil.copy2(inspect.getfile(intentionData), args.model_saved_name)
    else:
